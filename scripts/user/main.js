async function loadSwimmerInfo () {
    let user = await fetch("https://api.sagh-st.org/users/me", { method: "GET", headers});
    let user_data = await user.json();
    let swimmer = await fetch("https://api.sagh-st.org/swimmers/" + user_data['linked_swimmer'], { method: "GET", headers});
...
async function loadSwimmerTimes () {
    ...
    let swimmer = await fetch("https://api.sagh-st.org/swimmers/" + user_data['linked_swimmer'], { method: "GET", headers});
    let swimmer_data = await swimmer.json()
    let entries_resp = await fetch("https://api.sagh-st.org/swimmers/" + swimmer_data['id'] + "/entries", { method: "GET", headers})
