...
fetch("https://api.ghmvswim.org/meets")
...
function getMeet (param) {
    ...
    console.log(meetID)
    fetch("https://api.ghmvswim.org/meets/" + meetID)
        .then(response => response.json())
        .then(json => {
            console.log(json)
            ...
            fetch("https://api.ghmvswim.org/meets/" + meetID + "/entries/SAGH")
                    .then(resp => resp.json())
                    .then(json1 => {
                        ...
                        for (let event of events) {
                            ...
                            for (let times in results) {
                                ...
                                if (results[times]['relay']) {
                                    try {
                                        swimmers = `${results[times]['relay']['1']['last_name']}, ${results[times]['relay']['1']['first_name']} ${results[times]['relay']['1']['middle_name'].charAt(0)}<br>${results[times]['relay']['2']['last_name']}, ${results[times]['relay']['2']['first_name']} ${results[times]['relay']['2']['middle_name'].charAt(0)}<br>${results[times]['relay']['3']['last_name']}, ${results[times]['relay']['3']['first_name']} ${results[times]['relay']['3']['middle_name'].charAt(0)}<br>${results[times]['relay']['4']['last_name']}, ${results[times]['relay']['4']['first_name']} ${results[times]['relay']['4']['middle_name'].charAt(0)}`
                                    } catch {
                                    }
