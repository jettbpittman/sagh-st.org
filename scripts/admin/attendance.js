    ...
    fetch("https://api.sagh-st.org/teams/SAGH/roster/current")
        ...
        .then(json => {
            ...
            for (let swimmer in json) {
                ...
                row.innerHTML = `<td style='width: 25%; text-align: left; padding-left: 6px'>${json[swimmer]['last_name']}, ${json[swimmer]['first_name']} ${json[swimmer]['middle_name']}</td><td class="desktop" style='width: 5%'>${json[swimmer]['class']}</td><td class="desktop" style='width: 10%'>${json[swimmer]['dob']}</td><td class="desktop" style='width: 10%'>${usasID}</td><td class="mobile" style="width: 5%"><div class="mobile-hover" style="width: 100%; user-select: none"><span id="swimmer-info-dropdown"><b><i style="font-size: large">i</i></b></span><div class="swimmer-information"><table class="swimmer-info-table"><tr><td>Class</td><td>${json[swimmer]['class']}</td></tr><tr><td>DOB</td><td>${json[swimmer]['dob']}</td></tr><tr><td>USAS ID</td><td>${usasID}</td></tr></table></div></div></td><td style="width: 25%"><input type="radio" class="present" value="present" name="${json[swimmer]['id']}" checked><label for="present">Present</label>   <input type="radio" class="absent" value="absent" name="${json[swimmer]['id']}"><label for="absent">Absent</label></td>`
            }
        })
}
