    ...
    fetch("https://api.ghmvswim.org/teams/SAGH/roster/all", { headers: headers })
        ...
        .then(json => {
            ...
            for (let swimmer in json) {
                ...
                row.innerHTML = `<td style='width: 30%; ${color}'>${json[swimmer]['id']}</td><td style='width: 50%; text-align: left; padding-left: 5%;'><a style="${style} ${color}" href="swimmer.html?swimmer=` + json[swimmer]["id"] + `">${json[swimmer]['last_name']}, ${json[swimmer]['first_name']} ${json[swimmer]['middle_name']}</a></td><td style='width: 10%; ${color}'>${json[swimmer]['class']}</td><td style='width: 10%; ${color}'>${json[swimmer]['age']}</td>`
            }
        })
}
